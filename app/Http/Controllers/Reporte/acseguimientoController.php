  ...
  public function fichaConsolidado($id_tab_lapso, $id_ejecutor = null)
  {
    ...
    <tbody>
      ...
      foreach ($data2 as $data) {
        ...
        $actividad = tab_meta_fisica::select(
          'codigo',
          'nb_meta',
          'tx_prog_anual',
          'fecha_inicio',
          'fecha_fin',
          'tab_meta_fisica.nb_responsable',
          'de_unidad_medida as tx_unidades_medida',
          DB::raw('coalesce(tab_meta_fisica.nu_meta_modificada,0) as nu_meta_modificada'),
          DB::raw('coalesce(tab_meta_fisica.nu_meta_modificada_periodo,0) as nu_meta_modificada_periodo'),
          'de_municipio',
          'de_parroquia',
          'tab_meta_fisica.resultado',
          'tab_meta_fisica.observacion',
          DB::raw('coalesce(tab_meta_fisica.nu_meta_actualizada,0) as nu_meta_actualizada'),
          DB::raw('coalesce(tab_meta_fisica.nu_obtenido,0) as nu_obtenido')
        )
...
<tbody>';
      ...
      foreach ($data2 as $data) {
        ...
        $actividad = tab_meta_fisica::select(
          ...
          'codigo',
          'nb_meta',
          DB::raw('coalesce(mo_presupuesto,0) as mo_presupuesto'),
          DB::raw('coalesce(mo_modificado_anual,0) as mo_modificado_anual'),
          DB::raw('coalesce(mo_modificado,0) as mo_modificado'),
          DB::raw('coalesce(mo_actualizado_anual,0) as mo_actualizado_anual'),
          DB::raw('coalesce(mo_comprometido,0) as mo_comprometido'),
          DB::raw('coalesce(mo_causado,0) as mo_causado'),
          DB::raw('coalesce(mo_pagado,0) as mo_pagado'),
        ...
        $actividad_accion = tab_meta_fisica::select(
          'codigo',
          'nb_meta',
          DB::raw('coalesce(mo_presupuesto,0) as mo_presupuesto'),
          DB::raw('coalesce(mo_modificado_anual,0) as mo_modificado_anual'),
...
<td colspan="9" style="width: 100%;height: 30px;" align="justify"><b>OBSERVACIONES:</b>  ' . $data->de_observacion_003 . '</td>
      ...
      foreach ($data2 as $data) {
        ...
        $actividad = tab_meta_fisica::select(
          ...
          DB::raw('coalesce(mo_presupuesto,0) as mo_presupuesto'),
          DB::raw('coalesce(mo_modificado_anual,0) as mo_modificado_anual'),
          DB::raw('coalesce(mo_presupuesto,0) + coalesce(mo_modificado_anual,0) + coalesce(mo_modificado,0) as mo_actualizado_anual'),
          DB::raw('coalesce(mo_modificado,0) as mo_modificado'),
          'mo_comprometido',
          'mo_causado',
          'mo_pagado',
          ...
          'de_desvio',
          'tx_prog_anual',
          DB::raw('coalesce(tx_prog_anual::numeric) + coalesce(ac_seguimiento.tab_meta_fisica.nu_meta_modificada,0) + coalesce(ac_seguimiento.tab_meta_fisica.nu_meta_modificada_periodo,0) as nu_meta_actualizada'),
          DB::raw('coalesce(tab_meta_fisica.nu_meta_modificada_periodo,0) as nu_meta_modificada_periodo'),
          'nu_numero',
          DB::raw('coalesce(ac_seguimiento.tab_meta_fisica.nu_meta_modificada,0) as nu_meta_modificada'),
          'nu_original',
          'co_sector'
        )
          ->join('ac_seguimiento.tab_meta_financiera as t22', 'tab_meta_fisica.id', '=', 't22.id_tab_meta_fisica')
...
<tbody>';
        ...
        foreach ($actividad as $item) {
          ...
          $tab_meta_financiera = tab_meta_fisica::select(
            'codigo',
            'nb_meta',
            DB::raw('sum(coalesce(mo_presupuesto,0)) as mo_presupuesto'),
            DB::raw('sum(coalesce(mo_modificado_anual,0)) as mo_modificado_anual'),
...
<td colspan="9" style="width: 100%;height: 30px;" align="justify"><b>OBSERVACIONES:</b>  ' . $data->de_observacion_003 . '</td>
      ...
      foreach ($data2 as $data) {
        ...
        $actividad = tab_meta_fisica::select(
          ...
          'nb_meta',
          'co_partida',
          DB::raw('sum(distinct tx_prog_anual::numeric) as tx_prog_anual'),
  ...
  public function exportarAF($id_tab_lapso)
  {
    ...
    try {
      ...
      $data2 = tab_ac::select(
        ...
        ->where('ac_seguimiento.tab_ac.id_tab_lapso', '<=', $id_tab_lapso)
        ->where('ac_seguimiento.tab_ac.in_activo', '=', true)
        ->groupBy('ac_seguimiento.tab_ac.id_tab_ejercicio_fiscal')
        ->groupBy('ac_seguimiento.tab_ac.id_ejecutor')
        ->groupBy('ac_seguimiento.tab_ac.tx_ejecutor_ac')
        ->groupBy('t05.id_tab_tipo_ejecutor')
        ->groupBy('t06.de_tipo_ejecutor')
        ->groupBy('t02.nb_meta')
        ->groupBy('t02.codigo')
      ...
      $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G1', 'Meta Programada Poa')
        ->setCellValue('H1', 'Meta Modificada')
        ->setCellValue('I1', 'Meta Actualizada')
